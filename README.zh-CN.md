...
0.8），请使用 0.5.18。它在 TP 2 下对 Qwen3.5-27B 的打分同样是错的（JevBench 公开题准确率 0.39，0.5.18 上为 0.88）。35B-A3B 上所有 argmax 一致，但有一道接近平局的题（0.77 对 0.22）在 SGLang、vLLM、transformers
...
原版指令模型权重，`--prompt chat`，不做微调；27B 模型用 SGLang 0.5.18，其余用 0.5.9，跑在 A100 上。JevBench
v1.4.0（2026-09-23）的官方 Score 需要 308 道封存题，只有维护者能跑，所以下表给两组数：

- **公开题**：231 道公开题（standard 72、easy 48、hard 111）的准确率，与 49 个参赛系统在同样题目上公布的逐题结果一起排名。
- **v1.4 估计分**：用官方的 `composite_v14` 公式（它能重现所有已公布的分数）。有两项是假设：封存题准确率取公开题准确率与我们相差
  ±0.05 以内的一次前向参赛系统的 25% / 50% / 75% 分位；含封存题的校准分取 C13 × 0.885（参赛系统的中位比值）。成本用官方的
  自托管规则（同样权重的托管挂牌价 × 实测每题 668 个输入 token）。速度用官方的自托管调整（延迟 ×2 + 0.15 秒）。排名是在 71
  个有正式排名的系统里的位置。

| 模型 | 公开题准确率 | hard 111 | 公开题排名 /50 | v1.4 估计分 | v1.4 估计排名 /72 |
|---|---|---|---|---|---|
...
公开题上排在 Qwen3.5-27B 前面的三个系统是两个推理大模型（DeepSeek V4.1 Flash、GPT-5.6 Luna）和开了思考模式的
OpenJev。官方 v1.4 前五名在 54–63 分。差距来自封存题：一次前向的系统在封存题上只有 0.26–0.36（随机水平 0.293）。对
27B 模型，封存题项和公开题/封存题差距扣分把智能分从约 83 压到约 48，智能分低于 50 的门槛又把总分再压一次。
